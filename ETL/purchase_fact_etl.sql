...
WITH purchase_lines AS (
  SELECT
    ...
    -- Latest purchase price from normalized Items table.
    -- This is used for validation/comparison only.
...
SELECT
  ...
  ----------------------------------------------------------------------
  -- Fact Line_Total is calculated.
  ----------------------------------------------------------------------
  ROUND(pl.Quantity_Ordered * pl.Unit_Cost, 2) AS Line_Total,

  ----------------------------------------------------------------------
  -- Source Line_Total is kept for validation.
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- Comparison between PO UnitCost and latest item purchase price.
  -- For historical POs, differences can be normal.
  -- For newly created POs, this helps detect alignment issues.
  ----------------------------------------------------------------------
  CASE
...
BEGIN
  ...
  ------------------------------------------------------------------------
  -- 1. Required tables must not be empty
  ------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------
  -- 3. Staging validation in one scan
  ------------------------------------------------------------------------
  SELECT
    COUNT(*),
  ...
  ------------------------------------------------------------------------
  -- 4. Halt if staging count does not match source count
  ------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------
  -- 5. Halt if required keys are missing
  ------------------------------------------------------------------------
...
BEGIN
  ...
  ------------------------------------------------------------------------
  -- 2. Source row count for selected period
  ------------------------------------------------------------------------
  SELECT COUNT(*)
  INTO v_source_rows
  FROM Purchase_Orders po
  ...
  ------------------------------------------------------------------------
  -- 3. Staging validation for selected period
  ------------------------------------------------------------------------
  SELECT
    COUNT(*),
    NVL(SUM(CASE WHEN Supplier_Key IS NULL THEN 1 ELSE 0 END), 0),
